...
for tid = 1:size(metTests,1)
    waitbar(tid/size(metTests,1),h,'Assessing metabolic tests')
    model = modelOrg;
...
fprintf('\nStep 1\n')
for j = 1:length(HC)
    waitbar(j/length(HC),h,'Step 1 - finding support reactions')
    ...
    if flux.stat == 1 && flux.x(findRxnIDs(model,HC{j})) >= 1e-07
        MCpres(flux.x(MCids) > 1e-7) = true;
        NCpres(flux.x(NCids) > 1e-7) = true;
        HCpres(flux.x(HCids) > 1e-7) = true;
        prevHCtoMC = HCtoMC(j,:);
        prevHCtoNC = HCtoNC(j,:);
        HCtoMC(j,flux.x(MCids) > 1e-7) = 1;
        HCtoNC(j,flux.x(NCids) > 1e-7) = 1;
    ...
    elseif flux.stat ~= 1 || flux.x(findRxnIDs(model,HC{j})) < 1e-07
        HCtodel(j) = true;
    end
end
model = modelMod;
...
fprintf('\nStep 2.1\n')
for j = 1:length(MC)
    waitbar(j/length(MC),h,'Step 2.1 - Checking MC and NC co-occurence')
    model = modelMod;
    ...
    model.S(end,:) = cost;
    flux = optimizeCbModel(model,'min');
    if flux.stat ~= 1 || flux.x(findRxnIDs(model,MC{j})) < 1e-07
        MCtodel(j) = true;
    else
        while true
            prevMCxNC = MCxNC(j,:);
            MCxNC(j,flux.x(NCids) > 1e-7) = 1;
...
fprintf('\nStep 2.2\n')
for j = 1:length(MC)
    waitbar(j/length(MC),h,'Step 2.2 - Checking MC feasibility')
...
fprintf('\nStep 3\n')
for j = 1:length(HC)
    waitbar(j/length(HC),h,'Step 3 - Define remaining reactions')
    model = modelMod3;
    ...
    if flux.x(findRxnIDs(model,HC{j})) < 1e-07
        fprintf([HC{j} ' cannot carry flux alone\n'])
        continue
    end
    while true
        prevHCxOT = HCxOT(j,:);
        HCxOT(j,flux.x(OTids) > 1e-7) = 1;
